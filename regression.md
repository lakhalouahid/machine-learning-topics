...
```python title="Generation of the fake data"
...
```python title="Visualisation of the training data"
...
<!-- #region Defining the model to fit to the dataset -->
...
<!-- #region Fiting the model using the analytic solution -->
...
```python title="Fiting the model using the analytic solution"
...
```python title="Defining the cost and dcost function"
def cost(theta, x, y):
...
```python title="Full-Batch Gradient Descente Algorithm"
...
```python title="Mini-Batch Gradient Descent"
...
```python title="Nesterov accelerated gradient"
...
ncg_result = %timeit -o ncg_result = fmin_ncg(f=cost, x0=theta0, fprime=dcost, args=(xn, yn), disp=False)
ml_result = %timeit -o ml_result = least_squares(residuals, theta0, args=(xn, yn), method="lm")
```
